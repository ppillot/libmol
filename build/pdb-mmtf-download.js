  ...
  sheets.spreadsheets.values.get({
    ...
    if (rows.length == 0) {
      ...
      for (let i=0; i<rows.length; i++) {
        if (rows[i][6]==="mmtf" && rows[i][2]!=="?") {
          tabFiles.push({id: rows[i][0], code: rows[i][2], fileName: rows[i][2].toUpperCase() + '-' + rows[i][5] + '.mmtf.gz'})
        }
      }
      ...
      tabFiles.forEach(file => {
        ...
        const source = `http://mmtf.rcsb.org/v1.0/full/${file.code}.mmtf.gz`
